defmodule Csp.MinConflicts do
  ...
  def solve(csp, opts \\ []) do
    ...
    {status, assignment} =
      1..max_iterations
      |> Enum.reduce_while({:no_solution, assignment}, fn _iteration, {status, assignment} ->
        if Csp.consistent?(csp, assignment) do
          {:halt, {:solved, assignment}}
        else
          # TODO: how to prevent it from cycling non-stop?
          variable =
            Csp.conflicted(csp, assignment) |> Enum.random() |> IO.inspect(label: :variable)
          ...
          {:cont, {status, Map.put(assignment, variable, value)}}
        end
      end)
