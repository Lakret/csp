defmodule Csp.Problems do
  ...
  def sudoku(prefilled) do
    ...
    constraints = Enum.uniq(prefilled_constraints ++ column_constraints ++ row_constraints ++ box_constraints)
  ...
  def map_coloring() do
    ...
    constraints = Enum.map(inequalities, fn args -> {args, fn [state1, state2] -> state1 != state2 end} end)
  ...
  E.g., `2 => 4` assignment means that the queen is positioned on the row 2 and column 4.
  ...
  def nqueens(n \\ 8) when is_integer(n) do
    ...
    domains = Enum.map(variables, fn row_idx -> {row_idx, Enum.to_list(1..n)} end) |> Enum.into(%{})
  ...
  @doc """
  Returns an instance of [N-queens problem](https://en.wikipedia.org/wiki/Eight_queens_puzzle)
  for the specified `n`. **Note**, that this returns a more complicated representation of N Queens problem:
  ...
  def nqueens_slow(n \\ 8, use_row_placement_constraints \\ true) when is_integer(n) do
  ...
  """
  @spec pretty_print_nqueens_slow(Csp.assignment(), non_neg_integer()) :: :ok
  def pretty_print_nqueens_slow(assignment, n \\ 8) do
